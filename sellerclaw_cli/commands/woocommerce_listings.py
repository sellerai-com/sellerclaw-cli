...
SPECS = (
    Cmd(
        ...
        summary=(
            "List the store's WooCommerce listings from the SellerClaw mirror. `total` is the "
    ...
    Cmd(
        ...
        summary=(
            ...
            "in the background: the answer is the queued job and the command that reads it. "
            "Reading it gives `drafted` (one entry per product with what was decided, the "
            "`listing_ids` it became and `ready` for the group), `not_ready` (only the rows a "
            "publish would refuse, with their issues), and any product the shop had no category "
            "for; `--wait` holds on until then instead."
        ),
        body=(
            body_field(
                "product_ids",
