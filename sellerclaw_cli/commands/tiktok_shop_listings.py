...
# warmed on connect + refreshed periodically; pass --live on search to hit TikTok directly.
# The publish lifecycle (draft -> publish -> withdraw -> update) is TikTok-specific and category
# driven: publish is ASYNCHRONOUS — TikTok moderates every new product before it can sell, so a
# freshly published listing may sit "in review". A TikTok listing's remote id is "productId:skuId".
SPECS = (
    Cmd(
        "list",
        "GET",
        "/agent/stores/{store_id}/listings",
        summary="List the store's TikTok Shop listings from the SellerClaw mirror.",
        flags=(
            flag(
                "status",
    ...
    Cmd(
        ...
        summary=(
            "Aggregate stats over the store's TikTok Shop listings (row count, total & zero stock, "
    ...
    Cmd(
        ...
        flags=(
            ...
            flag(
                "type",
                help="Live-search field: sku (default) or remote_id (productId / productId:skuId); mirror ignores it.",
            ),
            flag(
                "live",
                type=bool,
    ...
    Cmd(
        ...
        summary=(
            "Update price and/or stock on existing TikTok products/SKUs "
    ...
    Cmd(
        ...
        summary=(
            ...
            "One draft row per product SKU. Set category_id here or later via 'update'; publish "
            "needs a leaf category (see 'tiktok-shop-store categories')."
        ),
        body=(
            body_field(
                "product_ids",
                repeatable=True,
                required=True,
                help="Catalog product UUIDs to stage as TikTok Shop drafts.",
            ),
            body_field(
                "category_id",
                help="Leaf TikTok category id to publish under (from 'tiktok-shop-store categories').",
            ),
        ),
    ),
    Cmd(
    ...
    Cmd(
        "withdraw",
        "POST",
        "/agent/tiktok-shop/stores/{store_id}/listings/withdraw",
        summary=(
            "Take published TikTok listings off the storefront (seller deactivation) "
    ...
    Cmd(
        ...
        body=(
            ...
            body_field(
                "category_id",
                help="Leaf TikTok category id (from 'tiktok-shop-store categories').",
            ),
            body_field(
                "sell_prices",
                type=dict,
...
app = build_group(
    NAME,
    "TikTok Shop listings: read (mirror, --live on search), sync price/stock, publish/withdraw.",
    SPECS,
)
